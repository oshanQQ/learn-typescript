...
export class User {
  ...
  static async all(): Promise<User[]> {
    ...
    const userRawDataList = await db.all<UserRawDataWithId[]>(
      "SELECT u.id, u.name, u.email, u.password, u.image_name, u.created_at, u.updated_at FROM users u"
    );
  ...
  static async find(userId: number): Promise<User | undefined> {
    ...
    const userRawData = await db.get<UserRawDataWithId>(
      "SELECT u.id, u.name, u.email, u.password, u.image_name, u.created_at, u.updated_at FROM users u WHERE u.id=?",
      [userId]
    );
